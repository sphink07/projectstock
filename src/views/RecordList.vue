<template>
  <div>
    <div style="height: 14vh; border-radius: 0px;" class="col00">
      <v-card class="col00">
        <v-row no-gutters>
          <v-col class="hidden-sm-and-down mt-7">
            <span style="font-size: 22px; margin-left: 20px; color: #ffffff">
              <v-icon color="#54f3ff">mdi-clipboard-text-outline</v-icon>
              รายการสินค้าที่เบิกแล้ว
            </span>
          </v-col>
          ...
          <v-col>
            ...
            <!-- --------------------------------------- sm size --------------------------------------- -->
            <v-text-field
              class="hidden-md-and-up mx-3 mt-5"
              v-model="search"
              append-icon="mdi-magnify"
              label="Search"
              solo
...
export default {
  ...
  async mounted() {
    await this.axios
      .post("https://firstmyapi.onrender.com/stockuser")
      .then((response) => {
